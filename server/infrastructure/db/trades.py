...
def update_trade_chat_pubkey(
    ...
    with get_cursor() as cursor:
        cursor.execute(
            sql, (json.dumps({"pubkey": chat_pubkey}), trade_id, identity_pubkey)
        )
...
def get_trade_with_chat_info(trade_id: str) -> dict[str, Any] | None:
    ...
    # 解析聊天公钥
    seller_chat_pubkey = (
        json.loads(row["seller_chat_pubkey"]) if row["seller_chat_pubkey"] else {}
    )
    buyer_chat_pubkey = (
        json.loads(row["buyer_chat_pubkey"]) if row["buyer_chat_pubkey"] else {}
    )
